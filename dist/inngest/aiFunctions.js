    ...
    try {
        ...
        const response = await step.run("generate-response", async () => {
          ...
          5. Considers safety and well-being`;
                ...
                const responseText = result.choices[0].message.content?.trim() || "I'm here to support you. Could you tell me more about what's on your mind?";
...
exports.analyzeTherapySession = client_1.inngest.createFunction({ id: "analyze-therapy-session" }, { event: "therapy/session.created" }, async ({ event, step }) => {
    try {
        ...
        Format the response as a JSON object. Return ONLY valid JSON with no markdown formatting.`;
            const result = await groq.chat.completions.create({
                model: "llama-3.3-70b-versatile",
                messages: [{ role: "user", content: prompt }],
                max_tokens: 1000,
