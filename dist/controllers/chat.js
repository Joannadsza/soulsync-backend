...
exports.getChatHistory = exports.getChatSession = exports.getSessionHistory = exports.analyseCrisis = exports.analyseSession = exports.sendMessage = exports.deleteSession = exports.getAllSessions = exports.createChatSession = void 0;
...
const crisisAnalysis_1 = require("../services/crisisAnalysis");
...
const getAllSessions = async (req, res) => {
    try {
        ...
        logger_1.logger.error("Error fetching all sessions:", error);
...
const deleteSession = async (req, res) => {
    try {
        ...
        await ChatSession_1.ChatSession.deleteOne({ sessionId });
...
const analyseSession = async (req, res) => {
    try {
        if (!req.user || !req.user.id) {
...
Required JSON (all fields required):
{
  ...
  "moodScore": <number 0-100 — overall session mood score where 100 is very positive>
        ...
        try {
            ...
            logger_1.logger.warn("analyseSession: JSON parse failed, using defaults", cleanAnalysis);
        }
        ...
        session.updatedAt = new Date();
        await session.save();
        logger_1.logger.info("Session analysed:", { sessionId, moodScore, emotionalState: analysis.emotionalState });
        res.json({ analysis });
    }
    catch (error) {
        logger_1.logger.error("Error in analyseSession:", error);
        res.status(500).json({ message: "Error analysing session", error: error instanceof Error ? error.message : "Unknown error" });
    }
};
exports.analyseSession = analyseSession;
const analyseCrisis = async (req, res) => {
    try {
        const { messages } = req.body;
        if (!messages || !Array.isArray(messages)) {
            return res.status(400).json({ error: "messages array is required" });
        }
        const result = await (0, crisisAnalysis_1.analyseCrisisSeverity)(messages);
        logger_1.logger.info("Crisis analysis result:", { userId: req.user?.id, severity: result.severity, action: result.recommendedAction });
        res.json(result);
    }
    catch (error) {
        logger_1.logger.error("Error in analyseCrisis:", error);
        res.status(500).json({ severity: 0, reason: "Analysis error", recommendedAction: "none" });
    }
};
exports.analyseCrisis = analyseCrisis;
